...
output_dir = 'barplots_comparison'
os.makedirs(output_dir, exist_ok=True)

metrics = ['min', 'median', 'mean', 'p90', 'p99', 'max']
...
def plot_comparison(section_name, data1, data2, label1='mp13', label2='mp25'):
    ...
    # Données mp13
    mins1 = [data1[t]['min'] for t in tests_common]
    maxs1 = [data1[t]['max'] for t in tests_common]
    heights1 = [maxs1[i] - mins1[i] for i in range(n)]
    bars1 = plt.bar(x - width/2, heights1, width, bottom=mins1, color='lightblue', alpha=0.7, label=label1, edgecolor='black')

    # Données mp25
    mins2 = [data2[t]['min'] for t in tests_common]
    maxs2 = [data2[t]['max'] for t in tests_common]
    heights2 = [maxs2[i] - mins2[i] for i in range(n)]
    ...
    plt.ylabel('Temps (ns)')
    plt.title(f"Comparaison min-max et percentiles pour la section '{section_name}'")
    ax = plt.gca()
    ...
    for i, t in enumerate(tests_common):
        ...
        for metric, color in colors.items():
            if metric in stats:
                val = stats[metric]
                ax.hlines(val, x_center - half_width, x_center + half_width, colors=color, linewidth=2)
                ax.text(x_center, val + max(maxs1)*0.02, f"{val}", ha='center', va='bottom', fontsize=7, color=color)
    ...
    for i, t in enumerate(tests_common):
        ...
        for metric, color in colors.items():
            if metric in stats:
                val = stats[metric]
                ax.hlines(val, x_center - half_width, x_center + half_width, colors=color, linewidth=2)
                ax.text(x_center, val + max(maxs2)*0.02, f"{val}", ha='center', va='bottom', fontsize=7, color=color)
...
for section in sections_common:
    plot_comparison(section, data_mp13[section], data_mp25[section])
